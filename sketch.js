...
let sketch0 = function(p){
  ...
  p.preload = function(){
    ...
    thisH1 = p.createElement("h1", 'Just you, <br>  yourself <br> and pixels');
  ...
  p.setup = function(){
    let initialCanvas = p.createCanvas(p.windowWidth, p.windowHeight);
...
let sketch2 = function(p){
  ...
  p.preload = function(){
    frame2 = p.loadImage("frame-02-05.png");
  }

  p.setup = function(){
    let secondCanvas = p.createCanvas(300, 250);
    secondCanvas.position(500, 100);
    capture2 = p.createCapture(p2.VIDEO);
    capture2.hide();

  }

  p.draw = function(){
    p.image(capture2,x2, y2);
    p.background('LemonChiffon');
    p.noStroke();
    p.fill(0);
    capture2.loadPixels();
